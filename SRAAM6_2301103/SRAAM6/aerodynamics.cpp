...
void Missile::def_aerodynamics()
{
    ...
    missile[159].init("stmarg",0,"Static margin (+stable, -unstable) - diameter","aerodynamics","diag","");
    missile[160].init("realq1",0,"First real root of airframe pitch dynamics - rad/s","aerodynamics","diag","");
    missile[161].init("realq2",0,"Second real root of airframe pitch dynamics - rad/s","aerodynamics","diag","");
    missile[162].init("wnq",0,"Natural frequency of airframe pitch dynamics - rad/s","aerodynamics","diag","");
    missile[163].init("zetq",0,"Damping of airframe pitch dynamics - NA","aerodynamics","diag","");
    ...
    missile[180].init("trcond","int",0,"Termination condition number","aerodynamics","diag","scrn,plot");
...
void Missile::init_aerodynamics()
{
	...
	//		 0	   >>> no termination condition reached <<<
	//		 1     trortho  kinematics		maximum orthogonality error
	//		 2     trmach	environment		minimum mach number
	//		 3     trdynm	environment		minimum dynamic pressure - lbf/ft^2
	//		 4     trload	aerodynamics    minimum load factor - g's
	//		 5     tralp	kinematics		maximum total incidence angle - rad
	//		 6     trtht	seeker			maximum pitch gimbal angle - rad - set in input file
	//		 7     trthtd	seeker			maximum pitch gimbal rate - rad/s - set in input file
	//		 8     trphid	seeker			maximum roll gimbal rate  - rad/s - set in input file
	//		 9     trate	seeker			maximum tracking rate - rad/s - set in input file

	double trortho(10e-4);
	double trmach(0.5);
...
void Missile::aerodynamics()
{
    ...
    ca=ca0+caa*alppx+cad*deff*deff+(double)(1-mprop)*caoff;
