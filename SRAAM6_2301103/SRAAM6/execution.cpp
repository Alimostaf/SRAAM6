...
//140924 Updated for Visual C++ V12, PZi
//150718 Modified for K2B, PZi
...
void execute(Vehicle &vehicle_list,Module *module_list,double sim_time,
			 ...
			 int num_missile,int num_target,int num_recce,ofstream &ftraj,const char *title,bool traj_merge,
...
void parse_plot_traj_csv(string *plot_files, int num_missiles, bool merge, string type);
...
int main(void) 
{
	...
	int num_recce=0; //number of recce objects
	...
	Document *doc_recce3 = NULL;  //array for documenting RECCE3 module-variables of 'input.asc'
	...
	bool document_recce3=false; //true if doc_recce3 was created
	...
	do
	{
		...
		bool document_recce=true; //write  to 'doc.asc' recce module-variables only once
		...
		number_objects(input,num_vehicles,num_missile,num_target,num_recce);
		...
		{
			//Loading pointers of the vehicle-object classes ('Missile','Target'.'Recce') into 
			...
			//Furthermore, it passes 'module_list', 'num_modules','num_target'and'num_recce'
			// to the 'Missile', 'Target' and 'Recce' constructors
			vehicle_type=set_obj_type(input,module_list,num_modules,num_target,num_recce);
			...
			{		
				...
				//composing documentation of 'flat6', 'missile', 'target' and 'recce' module-variables
				//but do it only once for MISSILE6, TARGET3 and RECCE3 objects
				if(strstr(options,"y_doc"))
				{
					...
					if(!strcmp(vehicle_name,"RECCE3")&&document_recce)
					{
						document_recce=false;
						int size=NFLAT3+NRECCE;
						doc_recce3=new Document[size];
						if(doc_recce3==0){cerr<<"*** Error: doc_recce3[] allocation failed *** \n";system("pause");exit(1);}
						vehicle_list[i]->document(fdoc,title,doc_recce3);
						document_recce3=true;
					}
				}
			}//end of initializing plot, stat files and documentation

			//loading packets into 'combus' communication bus
			combus[i]=vehicle_list[i]->loading_packet_init(num_missile,num_target,num_recce);
		...
		if(strstr(options,"y_traj")&&one_traj_banner)
		{
			...
			traj_banner(ftraj,combus,title,num_vehicles,num_missile,num_target,num_recce);
		...
		if(!nmc&&strstr(options,"y_doc")){
			...
			document_input(doc_missile6,doc_target3,doc_recce3);
			if(document_missile6)delete [] doc_missile6;
			if(document_target3)delete [] doc_target3;
			if(document_recce3)delete [] doc_recce3;
		}
		...
		execute(vehicle_list,module_list,sim_time,
				 ...
				 plot_ostream_list,combus,status,num_missile,num_target,num_recce,ftraj,title,
...
//zi150721	system("pause"); //do not use for SWEEP to stop run
	return 0;
}
///////////////////////////////////////////////////////////////////////////////	
////////////////////////// End of main ////////////////////////////////////////
...
//								and their respective type (missile, target or recce)
...
//				num_recce = number of 'Recce' objects
	...
	while (sim_time<=(end_time+int_step))
	{
		...
		for (int i=0;i<num_vehicles;i++)
		{
			...
			if(health==1)
			{
				...
				combus[i]=vehicle_list[i]->loading_packet(num_missile,num_target,num_recce);
