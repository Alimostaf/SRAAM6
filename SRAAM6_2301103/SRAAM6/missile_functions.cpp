...
void Missile::event(const char *options)
{
	...
	if(event_epoch)
	{
		...
		int flat6_size(0);
		int *missile_index_list=NULL;
		double *missile_value_list=NULL;
		int missile_size(0);
		int index(0);
		const char *integer=NULL;
		double value(0);
...
Packet Missile::loading_packet_init(int num_missile,int num_target,int num_recce)
{
	string id;
	char object[4];
...
Packet Missile::loading_packet(int num_missile,int num_target,int num_recce)
{
	int index(0);
	int i(0);
